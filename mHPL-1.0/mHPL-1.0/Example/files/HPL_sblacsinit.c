...
#define HPL_BLACSINIT_REAL hpl_sblacsinit__
#define HPL_BLACSINIT_LAYER hpl_sblacsinit_ 

#define HPL_PSGESV_REAL hpl_psgesv__
#define HPL_PSGESV_LAYER hpl_psgesv_

#define HPL_MATINIT_REAL hpl_smatinit__
#define HPL_MATINIT_LAYER hpl_smatinit_
...
#define HPL_BLACSINIT_REAL hpl_sblacsinit_ 
#define HPL_BLACSINIT_LAYER hpl_sblacsinit__ 

#define HPL_PSGESV_REAL hpl_psgesv_ 
#define HPL_PSGESV_LAYER hpl_psgesv__

#define HPL_MATINIT_REAL hpl_smatinit_ 
#define HPL_MATINIT_LAYER hpl_smatinit__
#endif


static HPL_T_grid grid ;
static HPL_ST_pmat mat ;



extern void psswap_( int *n, 
             float *x, int *ix, int *jx, int *descX, int *incx,
             float *y, int *iy, int *jy, int *descY, int *incy );

extern void pscopy_( int *n, 
             float *x, int *ix, int *jx, int *descX, int *incx,
             float *y, int *iy, int *jy, int *descY, int *incy );
...
extern void hpl_psgesv_(  int *pn, float *A, int *descA, int *ipiv, int *info );
extern void hpl_psgesv__(  int *pn, float *A, int *descA, int *ipiv, int *info );
...
void HPL_PSGESV_REAL(  int *pn, float *A, int *descA, int *ipiv, int *info )
{
...
#undef USE_HPL_PSINFO
   ...
   printf("before HPL_psinfo\n");
   HPL_psinfo( &test, &ns, nval, &nbs, nbval, &pmapping, &npqs, pval, qval,
...
align = ALIGNMENT;
   ...
   printf("equil %d \n", equil);
              ...
              if( L1notran != 0 )
              {
                 if( rpfa == HPL_LEFT_LOOKING ) algo.pffun = HPL_pspanllN;
                 else if( rpfa == HPL_CROUT   ) algo.pffun = HPL_pspancrN;
                 else                           algo.pffun = HPL_pspanrlN;

                 algo.rfact = rpfa = rfaval[irfa];
                 if( rpfa == HPL_LEFT_LOOKING ) algo.rffun = HPL_psrpanllN;
                 else if( rpfa == HPL_CROUT   ) algo.rffun = HPL_psrpancrN;
                 else                           algo.rffun = HPL_psrpanrlN;

                 if( Unotran != 0 ) algo.upfun = HPL_psupdateNN;
                 else               algo.upfun = HPL_psupdateNT;
              }
              else
              {
                 if( rpfa == HPL_LEFT_LOOKING ) algo.pffun = HPL_pspanllT;
                 else if( rpfa == HPL_CROUT   ) algo.pffun = HPL_pspancrT;
                 else                           algo.pffun = HPL_pspanrlT;

                 algo.rfact = rpfa = rfaval[irfa];
                 if( rpfa == HPL_LEFT_LOOKING ) algo.rffun = HPL_psrpanllT;
                 else if( rpfa == HPL_CROUT   ) algo.rffun = HPL_psrpancrT;
                 else                           algo.rffun = HPL_psrpanrlT;

                 if( Unotran != 0 ) algo.upfun = HPL_psupdateTN;
                 else               algo.upfun = HPL_psupdateTT;
              }
...
*/
  ...
  { 
    ...
    for (ia=0; ia < mat.n; ia++ ) {
      HPL_pselget( &grid, mat.A, ia,ja, mat.ld, mat.nb, &aij );
      if (isroot) {
 ...
 */
{
  int k = 0;
  int nn = 0, L = 0;
  ( void ) HPL_sall_reduce( (int *)(mat.global_ipiv), 
                 mat.n, HPL_INT, HPL_smax, grid.all_comm );
...
/*  Method 3 (Problem with the pscopy_)
{
...
//		global[kk] = (float)mat.global_ipiv[kk];
//		printf("global[%d] = %f\n", kk, global[kk]);
//	}
	
	int one = 1;
	pscopy_(&one, global, &one, &one, desc_global_ipiv, &one, ipiv, &one, &one, desc_ipiv, &one);
 ...
 */
  {
   ...
   for (i=0; i < mat.n; i++) {
       ix = 0; jx = i;
       HPL_pselget( &grid, mat.X, ix,jx, ld, mat.nb, &xij );
...
*/
  ...
  pscopy_( &(mat.n), mat.X, &ix,&jx, descX, &inc1, 
...
void HPL_PSGESV_LAYER( int *pn, float *A, int *descA, int *ipiv, int *info )
{
  HPL_PSGESV_REAL( pn, A, descA, ipiv, info );
}
