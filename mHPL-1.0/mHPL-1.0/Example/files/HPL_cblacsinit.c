...
#define HPL_BLACSINIT_REAL hpl_cblacsinit__
#define HPL_BLACSINIT_LAYER hpl_cblacsinit_ 

#define HPL_PCGESV_REAL hpl_pcgesv__
#define HPL_PCGESV_LAYER hpl_pcgesv_

#define HPL_MATINIT_REAL hpl_cmatinit__
#define HPL_MATINIT_LAYER hpl_cmatinit_
...
#define HPL_BLACSINIT_REAL hpl_cblacsinit_ 
#define HPL_BLACSINIT_LAYER hpl_cblacsinit__ 

#define HPL_PCGESV_REAL hpl_pcgesv_ 
#define HPL_PCGESV_LAYER hpl_pcgesv__

#define HPL_MATINIT_REAL hpl_cmatinit_ 
#define HPL_MATINIT_LAYER hpl_cmatinit__
#endif


static HPL_T_grid grid ;
static HPL_CT_pmat mat ;



extern void pcswap_( int *n, 
             ccomplex *x, int *ix, int *jx, int *descX, int *incx,
             ccomplex *y, int *iy, int *jy, int *descY, int *incy );

extern void pccopy_( int *n, 
             ccomplex *x, int *ix, int *jx, int *descX, int *incx,
             ccomplex *y, int *iy, int *jy, int *descY, int *incy );
...
extern void hpl_pcgesv_(  int *pn, ccomplex *A, int *descA, int *ipiv, int *info );
extern void hpl_pcgesv__(  int *pn, ccomplex *A, int *descA, int *ipiv, int *info );
...
void HPL_PCGESV_REAL(  int *pn, ccomplex *A, int *descA, int *ipiv, int *info )
{
...
8            memory alignment in ccomplex (> 0)
...
#undef USE_HPL_PSINFO

#ifdef USE_HPL_PSINFO
   printf("before HPL_pcinfo\n");
   HPL_pcinfo( &test, &ns, nval, &nbs, nbval, &pmapping, &npqs, pval, qval,
...
align = ALIGNMENT;
   ...
   printf("equil %d \n", equil);
              ...
              if( L1notran != 0 )
              {
                 if( rpfa == HPL_LEFT_LOOKING ) algo.pffun = HPL_pcpanllN;
                 else if( rpfa == HPL_CROUT   ) algo.pffun = HPL_pcpancrN;
                 else                           algo.pffun = HPL_pcpanrlN;

                 algo.rfact = rpfa = rfaval[irfa];
                 if( rpfa == HPL_LEFT_LOOKING ) algo.rffun = HPL_pcrpanllN;
                 else if( rpfa == HPL_CROUT   ) algo.rffun = HPL_pcrpancrN;
                 else                           algo.rffun = HPL_pcrpanrlN;

                 if( Unotran != 0 ) algo.upfun = HPL_pcupdateNN;
                 else               algo.upfun = HPL_pcupdateNT;
              }
              else
              {
                 if( rpfa == HPL_LEFT_LOOKING ) algo.pffun = HPL_pcpanllT;
                 else if( rpfa == HPL_CROUT   ) algo.pffun = HPL_pcpancrT;
                 else                           algo.pffun = HPL_pcpanrlT;

                 algo.rfact = rpfa = rfaval[irfa];
                 if( rpfa == HPL_LEFT_LOOKING ) algo.rffun = HPL_pcrpanllT;
                 else if( rpfa == HPL_CROUT   ) algo.rffun = HPL_pcrpancrT;
                 else                           algo.rffun = HPL_pcrpanrlT;

                 if( Unotran != 0 ) algo.upfun = HPL_pcupdateTN;
                 else               algo.upfun = HPL_pcupdateTT;
              }
...
*/
  ...
  { 
    ...
    for (ia=0; ia < mat.n; ia++ ) {
      HPL_pcelget( &grid, mat.A, ia,ja, mat.ld, mat.nb, &aij );
      if (isroot) {
 ...
 */
{
  int k = 0;
  int nn = 0, L = 0;
  ( void ) HPL_call_reduce( (int *)(mat.global_ipiv), 
                 mat.n, HPL_INT, HPL_cmax, grid.all_comm );
...
/*  Method 3 (Problem with the pccopy_)
{
...
//		global[kk] = (ccomplex)mat.global_ipiv[kk];
//		printf("global[%d] = %f\n", kk, global[kk]);
//	}
	
	int one = 1;
	pccopy_(&one, global, &one, &one, desc_global_ipiv, &one, ipiv, &one, &one, desc_ipiv, &one);
 ...
 */
  {
   ...
   for (i=0; i < mat.n; i++) {
       ix = 0; jx = i;
       HPL_pcelget( &grid, mat.X, ix,jx, ld, mat.nb, &xij );
...
*/
  ...
  pccopy_( &(mat.n), mat.X, &ix,&jx, descX, &inc1, 
...
void HPL_PCGESV_LAYER( int *pn, ccomplex *A, int *descA, int *ipiv, int *info )
{
  HPL_PCGESV_REAL( pn, A, descA, ipiv, info );
}
