...
#define HPL_BLACSINIT_REAL hpl_dblacsinit__
#define HPL_BLACSINIT_LAYER hpl_dblacsinit_ 

#define HPL_PDGESV_REAL hpl_pdgesv__
#define HPL_PDGESV_LAYER hpl_pdgesv_

#define HPL_MATINIT_REAL hpl_dmatinit__
#define HPL_MATINIT_LAYER hpl_dmatinit_
...
#define HPL_BLACSINIT_REAL hpl_dblacsinit_ 
#define HPL_BLACSINIT_LAYER hpl_dblacsinit__ 

#define HPL_PDGESV_REAL hpl_pdgesv_ 
#define HPL_PDGESV_LAYER hpl_pdgesv__

#define HPL_MATINIT_REAL hpl_dmatinit_ 
#define HPL_MATINIT_LAYER hpl_dmatinit__
#endif


static HPL_T_grid grid ;
static HPL_T_pmat mat ;



extern void pdswap_( int *n, 
             double *x, int *ix, int *jx, int *descX, int *incx,
             double *y, int *iy, int *jy, int *descY, int *incy );

extern void pdcopy_( int *n, 
             double *x, int *ix, int *jx, int *descX, int *incx,
             double *y, int *iy, int *jy, int *descY, int *incy );
...
extern void hpl_pdgesv_(  int *pn, double *A, int *descA, int *ipiv, int *info );
extern void hpl_pdgesv__(  int *pn, double *A, int *descA, int *ipiv, int *info );
...
void HPL_PDGESV_REAL(  int *pn, double *A, int *descA, int *ipiv, int *info )
{
...
#undef USE_HPL_PDINFO

#ifdef USE_HPL_PDINFO
   printf("before HPL_pdinfo\n");
...
*/
  ...
  { 
    ...
    for (ia=0; ia < mat.n; ia++ ) {
      HPL_pdelget( &grid, mat.A, ia,ja, mat.ld, mat.nb, &aij );
      if (isroot) {
 ...
 */
{
  int k = 0;
  int nn = 0, L = 0;
  ( void ) HPL_all_reduce( (int *)(mat.global_ipiv), 
                 mat.n, HPL_INT, HPL_max, grid.all_comm );
...
/*  Method 3 (Problem with the pdcopy_)
{
...
//		global[kk] = (double)mat.global_ipiv[kk];
//		printf("global[%d] = %f\n", kk, global[kk]);
//	}
	
	int one = 1;
	pdcopy_(&one, global, &one, &one, desc_global_ipiv, &one, ipiv, &one, &one, desc_ipiv, &one);
 ...
 */
  {
   ...
   for (i=0; i < mat.n; i++) {
       ix = 0; jx = i;
       HPL_pdelget( &grid, mat.X, ix,jx, ld, mat.nb, &xij );
...
*/
  ...
  pdcopy_( &(mat.n), mat.X, &ix,&jx, descX, &inc1, 
...
void HPL_PDGESV_LAYER( int *pn, double *A, int *descA, int *ipiv, int *info )
{
  HPL_PDGESV_REAL( pn, A, descA, ipiv, info );
}
